...
### Changed
- `rebuild-index` now embeds blueprints in batches of 200 instead of all at once,
  keeping peak memory bounded on large knowledge bases. Progress is printed per
  batch (`Indexed N/total...`) so long rebuilds are no longer silent.
